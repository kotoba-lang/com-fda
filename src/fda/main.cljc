(ns fda.main
  "Kotodama WASM entrypoint for the Fda clean-room actor (L4) — Clojure port.
  ...
  facts (`fda.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "fda_fil"
    :fields [:filerId :form :period :status]      :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "fda_ent"
    :fields [:name :ein :jurisdiction]            :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "fda_dat"
    :fields [:name :agency :updatedAt]            :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "fda_app"
    :fields [:applicantId :program :status]       :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "fda_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "fda_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"fda.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fda-compat" :tier tier :entities entities} 200])
